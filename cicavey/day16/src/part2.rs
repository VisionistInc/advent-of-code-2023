...
impl Beam {
    fn new(x: i16, y: i16, vx: i16, vy: i16) -> Self {
        Self {
            x: x,
            y: y,
            v: (vx, vy),
...
fn simulate(grid: &Grid, x: i16, y: i16, vx: i16, vy: i16) -> usize {
    let mut visited = VisitGrid::new(grid.w, grid.h);
    let mut beams = vec![Beam::new(x, y, vx, vy)];
    ...
    return visited.data.iter().map(|v| *v != 0).filter(|v| *v).count();
}
...
pub fn process(input: &str) -> String {
    ...
    let right = (0..grid.h)
        .map(|y| simulate(&grid, grid.w - 1, y, -1, 0))
        .max()
        .unwrap();
    ...
    let bottom = (0..grid.w)
        .map(|x| simulate(&grid, x, grid.h - 1, 0, -1))
        .max()
        .unwrap();
